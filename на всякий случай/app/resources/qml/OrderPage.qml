...
/*!
 * \brief Страница оформления заказа.
 *
 * Позволяет выбирать блюда из меню, добавлять их в заказ, указывать
 * количество, комментарий и номер столика. После оформления заказа
 * фиксирует его в базе данных и обновляет данные.
 */
Page {
    title: "Оформление заказа"

    //! Полное меню, полученное с сервера/БД.
    property var fullMenuModel: []
    //! Отфильтрованное меню для отображения.
    ListModel { id: filteredMenuModel }
    //! Текущий состав заказа.
    ListModel { id: orderModel }
    //! Индекс выбранного блюда в filteredMenuModel.
    ...
    //! Загружает меню через dbWorker и применяет фильтры.
    function loadFoods() {
    ...
    //! Применяет фильтры по категории и доступности.
    ...
    /*!
     * \brief Добавляет блюдо в заказ (с проверкой и списанием).
     * \param foodId ID блюда.
     * \param quantity Количество.
     * \param dish Объект блюда (содержит name, price и т.д.).
     */
    ...
    // Диалог предупреждения о падении остатков ниже порога
    Dialog {
        id: warningDialog
