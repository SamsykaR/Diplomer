...
Page {
    ...
    property int selectedReportIndex: 0 //!< Индекс выбранного типа отчёта (0 – выручка, 1 – популярность, 2 – малые остатки, 3 – чеки).
    ListModel { id: dishModel }   //!< Модель блюд для фильтра.
    ListModel { id: waiterModel } //!< Модель официантов для фильтра.
    property int selectedDishId: 0    //!< ID выбранного блюда (0 – все).
    property int selectedTable: 0     //!< Номер столика (0 – все).
    property int selectedWaiterId: 0  //!< ID выбранного официанта (0 – все).
    ...
    /*!
     * \brief Загружает данные для выпадающих списков (блюда, официанты).
     */
