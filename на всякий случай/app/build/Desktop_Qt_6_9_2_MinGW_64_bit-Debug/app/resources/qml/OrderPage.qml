...
Page {
    ...
    property int orderId: -1   // обязательно int

    // Модели данных
    property var fullMenuModel: []       // полное меню с сервера
    ListModel { id: filteredMenuModel } // отфильтрованное меню для отображения
    ListModel { id: orderModel }        // корзина (текущий заказ)
    ...
    // Перезагрузка данных (синхронизация)
    function reloadData() {
